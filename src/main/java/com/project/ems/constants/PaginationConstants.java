...
public class PaginationConstants {
    ...
    public static final String MENTOR_FILTER_QUERY = "select m from Mentor m where lower(concat(m.name, ' ', m.email, ' ', m.mobile, ' ', m.address, ' ', m.birthday, ' ', m.role.authority, ' ', m.employmentType, ' ', m.position, ' ', m.grade, ' ', m.supervisingMentor.name, ' ', m.nrTrainees, ' ', m.maxTrainees, ' ', m.isTrainingOpen)) like %:key%";
