...
public class PaginationConstants {
    ...
    public static final String MENTOR_FILTER_QUERY = "select m from Mentor m left join m.supervisingMentor sm where lower(concat(m.name, ' ', m.email, ' ', m.mobile, ' ', m.address, ' ', m.birthday, ' ', m.role.authority, ' ', m.employmentType, ' ', m.position, ' ', m.grade, ' ', coalesce(sm.name, ''), ' ', m.nrTrainees, ' ', m.maxTrainees, ' ', m.openForTraining)) like %:key%";
