...
public class PaginationConstants {
    ...
    public static final String EMPLOYEE_FILTER_QUERY = "select e from Employee e where lower(concat(e.name, ' ', e.email, ' ', e.mobile, ' ', e.address, ' ', e.birthday, ' ', e.role.type, ' ', e.employmentType, ' ', e.position, ' ', e.grade, ' ', e.mentor.name)) like %:key%";
    ...
    public static final String MENTOR_FILTER_QUERY = "select m from Mentor m left join m.supervisingMentor sm where lower(concat(m.name, ' ', m.email, ' ', m.mobile, ' ', m.address, ' ', m.birthday, ' ', m.role.type, ' ', m.employmentType, ' ', m.position, ' ', m.grade, ' ', coalesce(sm.name, ''), ' ', m.nrTrainees, ' ', m.maxTrainees, ' ', m.openForTraining)) like %:key%";
